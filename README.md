# Moki&Julio — 2,107 new best-known packings of equal circles

**2,107 packings of equal circles that beat the best-known records** listed on
...
## Cumulative since version 1.0 (as of version 1.4)

- **2,107 records** (v1.0: 1,933). **174 sizes where we had no record before**; **883 of the v1.0 records improved
  ...
  exact checkers (2107 / 2107), and Packomania's printed radius for every claimed size was re-derived from its own coordinate file in the
...
- **Local optimality certificates** (`local_optimality/<table>.zip`, one JSON per packing: kept constraints and contact forces).
  For **1,190** of the 2,107 packings, two independently written checkers (`checkers/lopt.py`, float-rigorous;
  `checkers/verify_lopt.py`, exact rational in ℚ(√2)) prove: every feasible packing whose load-bearing circles and radius lie
  within an explicit distance ρ of ours (median ρ = 9.0e-09 r) has radius at most ours + Δ (Δ ≤ 6e-26 r in every case,
  median 3e-44 r), and a true local maximum lies within t₀ of ours. In plain words: no small nudge beats these packings.
  Of the other 917: 900 carry a first-order flex (a sliding or buckling motion the first-order theorem cannot
  exclude) and 17 could not be certified by this method; none of them is claimed locally optimal. Verdict, ρ
...
`MANIFEST.csv`. A record counts only if **both** say `IMPROVES`. Expected output: `TOTAL: 2107 / 2107 verified by both checkers`.
...
1,767 new arrangements (a held circle moved ≥ 5 % of a radius and the gain is ≥ 10⁻⁶), 140 refinements of the published
arrangement, 200 small gains we do not claim as new structures.

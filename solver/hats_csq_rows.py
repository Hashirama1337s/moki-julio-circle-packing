...
BOTH O(N) exact checkers. -> cand_big_hp/csq/csq_<N>.txt + .json (tag csq_rows)   usage: py -3.11 hats_csq_rows.py
"""
import os, sys, json, subprocess, time
HERE = os.path.dirname(os.path.abspath(__file__)); sys.path.insert(0, HERE); sys.path.insert(0, os.path.join(HERE, "engine"))
import numpy as np, mpmath as mp
from decimal import Decimal, ROUND_FLOOR, getcontext
import csq_constructions as G, lai_compare as L, certify_big, hats_flagship as HF
getcontext().prec = 60
T = HF.table("csq"); T2, T7 = L.load(); out = open(os.path.join(HERE, "out", "hats_csq_rows.jsonl"), "a"); kept = 0
for n in sorted((k for k in T if k > 1000), reverse=True):          # descending: hats_csq_polish owns N <= 2000 until it finishes
    ...
    if ok and r_claim > ours:
        tmp = f + ".b.tmp"; c = best[2]
        open(tmp, "w", newline="\n").write(f"r {r_claim}\n" + "".join(f"{format(Decimal(repr(float(x))), 'f')} {format(Decimal(repr(float(y))), 'f')}\n" for x, y in c))
        a = certify_big.check("square", tmp, n, T[n])
        o = subprocess.run([sys.executable, os.path.join(HERE, "..", "checkers", "verify_exact_big.py"), "square", tmp, str(n), T[n]], capture_output=True, text=True).stdout
        ...
        if a == "IMPROVES" and b == "IMPROVES":
            ...
            json.dump({"shelf": "csq", "N": n, "tag": "csq_rows", "how": f"construction {best[1]} (csq_constructions.py)",
